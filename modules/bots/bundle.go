...
const (
	...
	PlaceholderEnvFile  = "__TAKAN_ENVFILE__"
)

// BundleEnvFile is where provisioning writes the daemon environment file. The
// source config.toml documents its own env path in a comment, so that path is
// templated too rather than left pointing at the source machine's home.
func BundleEnvFile(instance string) string { return "/etc/" + instance + "/" + instance + ".env" }
...
func templatize(text, name, slug, dataDir, envFile string) string {
	if text == "" {
		return text
	}
	if envFile != "" {
		text = strings.ReplaceAll(text, envFile, PlaceholderEnvFile)
	}
	if dataDir != "" {
...
func render(text, name, instance, dataDir string) string {
	return strings.NewReplacer(
		PlaceholderEnvFile, BundleEnvFile(instance),
...
// instanceHeader is the [instance] block prepended to a rendered daemon config
// that has none. The source machine relies on the daemon's built-in default
// ("Atlas"), so without this every provisioned bot would introduce itself as
// Atlas and register under that name.
func instanceHeader(name string) string {
	return "# Generated by Takan for this bot instance.\n[instance]\nname = \"" + name + "\"\n\n"
}

// renderAgentConfig renders the daemon config.toml and guarantees it names this
// bot, whether or not the source config carried an [instance] section.
func renderAgentConfig(cfg, name, instance, dataDir string) string {
	out := render(cfg, name, instance, dataDir)
	if out == "" {
		return out
	}
	if instanceName(out) == DefaultInstanceName && !hasInstanceName(out) {
		return instanceHeader(name) + out
	}
	return out
}

// hasInstanceName reports whether [instance] name is set explicitly.
func hasInstanceName(cfg string) bool {
	section := ""
...
func (b *Bundle) Tar(name, instance, dataDir string) ([]byte, error) {
	...
	if err := write("data/config.toml", 0o600, renderAgentConfig(b.AgentConfig, name, instance, dataDir)); err != nil {
		return nil, err
	}
...
func ReadBundle(p ImportPaths) (*ImportResult, error) {
	...
	b.AgentConfig = templatize(agentCfg, res.Name, res.Slug, agentData, p.EnvFile)
	b.AgentsMD = templatize(guide, res.Name, res.Slug, agentData, p.EnvFile)
	res.Bundle = b
	return res, nil
}

...
Takan is a **single-operator personal hub**. The operator stores secrets (session encryption key, Resend keys, Mercadona credentials, agent tokens). Extra `users` rows in an old database cannot unlock the panel; they are leftover storage, not a product tenant.
...
In scope: authentication/authorization bugs, secret disclosure, remote code execution **on the hub** without intended privileges, and XSS in the panel that can steal sessions.

Out of scope (unless you show impact on the hub / instance-password boundary): issues that only affect an operator who deliberately installs `takan-agent` and runs AI/shell tools on their own machine; Mercadona or Resend account abuse using the operator’s own credentials; dependency CVEs with no practical exploit path in Takan’s usage.
